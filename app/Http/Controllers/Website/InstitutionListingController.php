...
class InstitutionListingController extends Controller
{
    public function index(Request $request)
    {
        ...
        $institutions = $query->orderBy('is_featured', 'desc')
        ...
        $provinces = Institution::active()->whereNotNull('province')->distinct()->orderBy('province')->pluck('province');
        $faculties = Faculty::where('is_active', true)->orderBy('name')->get(['id', 'name', 'slug']);

        return view('website.institutions.index', compact(
            'institutions', 'types', 'provinces', 'faculties'
        ));
    }
}
