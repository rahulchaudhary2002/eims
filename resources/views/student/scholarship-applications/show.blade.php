...
<section class="bg-[#f7fafc] pt-12 pb-20">
    <div class="container max-w-7xl mx-auto px-4">
        <div class="max-w-2xl">
            @php $sc = ['pending' => 'bg-gray-100 text-gray-600', 'under_review' => 'bg-yellow-100 text-yellow-700', 'approved' => 'bg-green-100 text-green-700', 'rejected' => 'bg-red-100 text-red-700', 'withdrawn' => 'bg-gray-100 text-gray-500'];
            $schStatus = $scholarshipApplication->scholarship_status; @endphp
            <div class="bg-white rounded-xl shadow-[0_5px_15px_rgba(0,0,0,0.08)] border border-gray-200 overflow-hidden">
                <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
                    ...
                    <span class="text-xs font-semibold px-2.5 py-1 rounded-full {{ $sc[$schStatus] ?? '' }}">
                        {{ \App\Models\Application::SCHOLARSHIP_STATUSES[$schStatus] ?? ($schStatus ?? 'Pending') }}
                    </span>
                </div>
                <dl class="divide-y divide-gray-50">
                    @if($scholarshipApplication->scholarship_approved_amount)
                    <div class="flex px-6 py-3">
                        <dt class="text-sm text-gray-500 w-44 shrink-0">Approved Amount</dt>
                        <dd class="text-sm font-bold text-green-600">NPR {{ number_format($scholarshipApplication->scholarship_approved_amount) }}</dd>
                    </div>
                    @endif
                    @if($scholarshipApplication->scholarship_remarks)
                    <div class="px-6 py-3">
                        <dt class="text-sm text-gray-500 mb-1">Remarks</dt>
                        <dd class="text-sm text-gray-700">{{ $scholarshipApplication->scholarship_remarks }}</dd>
                    </div>
                    @endif
                    <div class="flex px-6 py-3">
