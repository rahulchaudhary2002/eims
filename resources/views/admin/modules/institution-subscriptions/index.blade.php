...
@section('page-title', 'Institution Subscriptions')

@section('content')
<div class="space-y-5">
    <x-admin.page-header
        title="Institution Subscriptions"
        subtitle="Active and historical subscriptions for institutions."
        :breadcrumbs="[
            ['label' => 'Dashboard', 'route' => 'admin.dashboard'],
            ['label' => 'Institution Subscriptions'],
        ]">
        <x-slot:actions>
            <a href="{{ route('admin.institution-subscriptions.create') }}" class="btn btn-primary">
    ...
    <div class="eims-card p-4">
        <form method="GET" action="{{ route('admin.institution-subscriptions.index') }}" class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-3 items-end">
            <div>
            ...
            <div>
                ...
                <select name="billing_cycle" class="form-control">
                    ...
                    @foreach($billingCycles as $value => $label)
                        <option value="{{ $value }}" {{ request('billing_cycle') === $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="date" name="ends_to" value="{{ request('ends_to') }}" class="form-control">
            </div>
            <div class="flex gap-2 md:col-span-3 xl:col-span-4">
                <button type="submit" class="btn btn-primary">Filter</button>
                <a href="{{ route('admin.institution-subscriptions.index') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="eims-card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="eims-table">
                ...
                <tbody>
                    @forelse($subscriptions as $sub)
                        <tr>
                            ...
                            <td class="text-xs text-slate-500">
                                @if($sub->ends_at)
                                    <span class="{{ $sub->ends_at->isPast() && $sub->status === 'active' ? 'text-red-600 font-medium' : '' }}">
                            ...
                            <td><span class="badge">{{ $statuses[$sub->status] ?? $sub->status }}</span></td>
                            <td>
                                <div class="flex items-center gap-1">
                                    ...
                                    <form action="{{ route('admin.institution-subscriptions.destroy', $sub) }}" method="POST" onsubmit="return confirm('Delete this subscription?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center text-slate-400 py-10">No subscriptions found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        @if($subscriptions->hasPages())
            <div class="px-4 py-3 border-t border-slate-100">{{ $subscriptions->links() }}</div>
        @endif
    </div>
</div>
@endsection
