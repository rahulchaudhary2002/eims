...
<div class="space-y-5">
    <x-admin.page-header
        ...
        subtitle="Scheduled counseling sessions between students and counselors."
        :breadcrumbs="[
            ['label' => 'Dashboard', 'route' => 'admin.dashboard'],
            ['label' => 'Counseling Sessions'],
        ]">
        <x-slot:actions>
            <a href="{{ route('admin.counseling-sessions.create') }}" class="btn btn-primary">
    ...
    <div class="eims-card p-4">
        <form method="GET" action="{{ route('admin.counseling-sessions.index') }}" class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-3 items-end">
            <div>
            ...
            <div>
                <label class="form-label text-xs">Counselor</label>
                <select name="counselor_id" class="form-control">
                    <option value="">All Counselors</option>
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ request('counselor_id') == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="form-label text-xs">Mode</label>
            ...
            <div>
                <label class="form-label text-xs">Scheduled From</label>
                <input type="date" name="date_from" value="{{ request('date_from') }}" class="form-control">
            </div>
            <div>
                <label class="form-label text-xs">Scheduled To</label>
                <input type="date" name="date_to" value="{{ request('date_to') }}" class="form-control">
            </div>
            <div class="flex gap-2 md:col-span-3 xl:col-span-4">
                <button type="submit" class="btn btn-primary">Filter</button>
                <a href="{{ route('admin.counseling-sessions.index') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="eims-table">
                ...
                <tbody>
                    @forelse($sessions as $session)
                        <tr>
                            ...
                            <td class="text-sm">
                                ...
                                @if($session->scheduled_at->isToday()) <div class="text-xs text-amber-600">Today</div> @endif
                            </td>
                            <td><span class="badge">{{ $statuses[$session->status] ?? $session->status }}</span></td>
                            <td>
                                <div class="flex items-center gap-1">
                                    @if($session->status === 'pending')
                                    <form action="{{ route('admin.counseling-sessions.update-status', $session) }}" method="POST">
                                        @csrf @method('PATCH')
                                        <input type="hidden" name="status" value="scheduled">
                                        <button type="submit" class="btn-icon text-green-600 hover:bg-green-50 border border-green-200" title="Approve">
                                    ...
                                    @endif
                                    <a href="{{ route('admin.counseling-sessions.show', $session) }}" class="btn-icon btn-icon-view" title="View">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                    </a>
                                    <a href="{{ route('admin.counseling-sessions.edit', $session) }}" class="btn-icon btn-icon-edit" title="Edit">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/></svg>
                                    </a>
                                    <form action="{{ route('admin.counseling-sessions.destroy', $session) }}" method="POST" onsubmit="return confirm('Delete this session?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="text-center text-slate-400 py-10">No counseling sessions found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
