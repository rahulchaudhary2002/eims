...
@section('content')
<div class="space-y-5">
    <x-admin.page-header title="Edit Program" subtitle="Update details for: {{ $program->name }}"
        :breadcrumbs="[['label'=>'Dashboard','route'=>'admin.dashboard'],['label'=>'Programs','route'=>'admin.program.index'],['label'=>'Edit']]">
        <x-slot:actions>
    ...
    <x-admin.form-card title="Program Details">
        <form action="{{ route('admin.program.update', $program) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <x-admin.form-input name="name" label="Program Name" :value="old('name', $program->name)" required />
                <x-admin.form-input name="code" label="Code" :value="old('code', $program->code)" />
                <x-admin.form-select name="category_id" label="Program Category" required>
                    <option value="">Select Category</option>
                    @foreach($categories as $category)
                    <option value="{{ $category->id }}" {{ old('category_id', $program->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                    @endforeach
                </x-admin.form-select>
                ...
                <x-admin.form-select name="affiliation_id" label="Affiliation">
                    ...
                    <option value="{{ $affiliation->id }}" {{ old('affiliation_id', $program->affiliation_id) == $affiliation->id ? 'selected' : '' }}>{{ $affiliation->name }}</option>
                    @endforeach
                </x-admin.form-select>
                <x-admin.form-input name="fee" label="Fee (NPR)" :value="old('fee', $program->fee)" />
                <x-admin.form-input name="duration" label="Duration" :value="old('duration', $program->duration)" />
                <div class="md:col-span-2">
                    <x-admin.form-textarea name="description" label="Description" :value="old('description', $program->description)" :rows="3" />
                </div>
                <div class="md:col-span-2 flex items-center gap-3 pt-1">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $program->is_active) ? 'checked' : '' }} class="w-4 h-4 text-primary-600 border-slate-300 rounded focus:ring-primary-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="level_id" id="level_id"
                            ...
                            <option value="">Select Level</option>
                            @foreach($levels as $level)
                            <option value="{{ $level->id }}" {{ old('level_id', $program->level_id) == $level->id ? 'selected' : '' }}>
                                {{ $level->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="affiliation_id" id="affiliation_id"
                            ...
                            <option value="">Select Affiliation (Optional)</option>
                            @foreach($affiliations as $affiliation)
                            <option value="{{ $affiliation->id }}" {{ old('affiliation_id', $program->affiliation_id) == $affiliation->id ? 'selected' : '' }}>
                                {{ $affiliation->name }}
                            </option>
                            @endforeach
                        </select>
