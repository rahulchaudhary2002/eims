...
@section('content')
<div class="space-y-5">
    <x-admin.page-header title="Edit Level" subtitle="Update details for: {{ $level->name }}"
        :breadcrumbs="[['label'=>'Dashboard','route'=>'admin.dashboard'],['label'=>'Levels','route'=>'admin.level.index'],['label'=>'Edit']]">
        <x-slot:actions>
    ...
    <x-admin.form-card title="Level Details">
        <form action="{{ route('admin.level.update', $level) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <x-admin.form-input name="name" label="Level Name" :value="old('name', $level->name)" required />
                <x-admin.form-input name="code" label="Code" :value="old('code', $level->code)" />
                <x-admin.form-input name="order" label="Display Order" type="number" :value="old('order', $level->order)" help="Lower numbers appear first" required />
                <div class="md:col-span-2">
                    <x-admin.form-textarea name="description" label="Description" :value="old('description', $level->description)" :rows="3" />
                </div>
                <div class="md:col-span-2 flex items-center gap-3 pt-1">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $level->is_active) ? 'checked' : '' }} class="w-4 h-4 text-primary-600 border-slate-300 rounded focus:ring-primary-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3"
                            ...
                            placeholder="Enter level description">{{ old('description', $level->description) }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-sm mt-1 flex items-center">
                            <x-lucide-alert-circle class="w-4 h-4 mr-1" />
                            {{ $message }}
