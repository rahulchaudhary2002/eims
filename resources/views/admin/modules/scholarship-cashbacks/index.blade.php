...
@section('title', 'Scholarship Cashbacks')
@section('page-title', 'Scholarship Cashbacks')

@section('content')
<div class="space-y-5">
    <x-admin.page-header
        title="Scholarship Cashbacks"
        subtitle="Manage cashbacks disbursed to students linked to commission invoices."
        :breadcrumbs="[
            ['label' => 'Dashboard', 'route' => 'admin.dashboard'],
            ['label' => 'Scholarship Cashbacks'],
        ]">
        <x-slot:actions>
            <a href="{{ route('admin.scholarship-cashbacks.create') }}" class="btn btn-primary">
    ...
    <div class="eims-card p-4">
        <form method="GET" action="{{ route('admin.scholarship-cashbacks.index') }}" class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-3 items-end">
            <div>
            ...
            <div>
                ...
                <select name="commission_invoice_id" class="form-control">
                    ...
                    @foreach($invoices as $invoice)
                        <option value="{{ $invoice->id }}" {{ request('commission_invoice_id') == $invoice->id ? 'selected' : '' }}>{{ $invoice->invoice_number }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="date" name="paid_from" value="{{ request('paid_from') }}" class="form-control">
            </div>
            <div>
                <label class="form-label text-xs">Paid To</label>
                <input type="date" name="paid_to" value="{{ request('paid_to') }}" class="form-control">
            </div>
            <div class="flex gap-2 md:col-span-3 xl:col-span-4">
                <button type="submit" class="btn btn-primary">Filter</button>
                <a href="{{ route('admin.scholarship-cashbacks.index') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="eims-card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="eims-table">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    ...
                                    <form action="{{ route('admin.scholarship-cashbacks.destroy', $cashback) }}" method="POST" onsubmit="return confirm('Delete this scholarship cashback?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="text-center text-slate-400 py-10">No scholarship cashbacks found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        @if($cashbacks->hasPages())
            <div class="px-4 py-3 border-t border-slate-100">{{ $cashbacks->links() }}</div>
        @endif
    </div>
</div>
@endsection
