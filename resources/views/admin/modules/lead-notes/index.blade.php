...
@section('page-title', 'Lead Notes')

@section('content')
<div class="space-y-5">
    <x-admin.page-header
        title="Lead Notes"
        subtitle="Notes attached to inquiry records."
        :breadcrumbs="[
            ['label' => 'Dashboard', 'route' => 'admin.dashboard'],
            ['label' => 'Lead Notes'],
        ]">
        <x-slot:actions>
            <a href="{{ route('admin.lead-notes.create') }}" class="btn btn-primary">
    ...
    <div class="eims-card p-4">
        <form method="GET" action="{{ route('admin.lead-notes.index') }}" class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-3 items-end">
            <div>
            ...
            <div class="flex gap-2 md:col-span-3 xl:col-span-4">
                ...
                <a href="{{ route('admin.lead-notes.index') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
                ...
                <tbody>
                    @forelse($leadNotes as $note)
                        <tr>
                            <td>
                                @if($note->inquiry)
                                    <a href="{{ route('admin.inquiries.show', $note->inquiry) }}" class="text-blue-600 hover:underline font-medium text-sm">
                                        {{ $note->inquiry->name }}
                                    </a>
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    <a href="{{ route('admin.lead-notes.show', $note) }}" class="btn-icon btn-icon-view" title="View">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-slate-400 py-10">No lead notes found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($leadNotes->hasPages())
            <div class="px-4 py-3 border-t border-slate-100">{{ $leadNotes->links() }}</div>
        @endif
    </div>
</div>
@endsection
