...
@section('page-title', 'Commission Invoices')

@section('content')
<div class="space-y-5">
    <x-admin.page-header
        title="Commission Invoices"
        subtitle="Manage commission invoices linked to admissions and referral agreements."
        :breadcrumbs="[
            ['label' => 'Dashboard', 'route' => 'admin.dashboard'],
            ['label' => 'Commission Invoices'],
        ]">
        <x-slot:actions>
            <a href="{{ route('admin.commission-invoices.create') }}" class="btn btn-primary">
    ...
    <div class="eims-card p-4">
        <form method="GET" action="{{ route('admin.commission-invoices.index') }}" class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-3 items-end">
            <div>
            ...
            <div>
                <label class="form-label text-xs">Admission</label>
                <select name="admission_id" class="form-control">
                    <option value="">All Admissions</option>
                    @foreach($admissions as $admission)
                        <option value="{{ $admission->id }}" {{ request('admission_id') == $admission->id ? 'selected' : '' }}>{{ $admission->admission_number }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="form-label text-xs">Referral Agreement</label>
            ...
            <div>
                <label class="form-label text-xs">Commission Type</label>
                <select name="commission_type" class="form-control">
                    <option value="">All Types</option>
                    @foreach($commissionTypes as $value => $label)
                        <option value="{{ $value }}" {{ request('commission_type') === $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div>
                <label class="form-label text-xs">Invoice Date From</label>
                <input type="date" name="invoice_date_from" value="{{ request('invoice_date_from') }}" class="form-control">
            </div>
            <div>
                <label class="form-label text-xs">Invoice Date To</label>
                <input type="date" name="invoice_date_to" value="{{ request('invoice_date_to') }}" class="form-control">
            </div>
            <div>
                <label class="form-label text-xs">Due Date From</label>
                <input type="date" name="due_date_from" value="{{ request('due_date_from') }}" class="form-control">
            </div>
            <div>
                <label class="form-label text-xs">Due Date To</label>
            ...
            <div class="flex gap-2 md:col-span-3 xl:col-span-4">
                ...
                <a href="{{ route('admin.commission-invoices.index') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="eims-table">
                ...
                <tbody>
                    @forelse($invoices as $invoice)
                        <tr>
                            <td>
                                <a href="{{ route('admin.commission-invoices.show', $invoice) }}" class="font-semibold text-blue-600 hover:underline font-mono text-sm">
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    ...
                                    <form action="{{ route('admin.commission-invoices.destroy', $invoice) }}" method="POST" onsubmit="return confirm('Delete this commission invoice?')">
                                        @csrf @method('DELETE')
