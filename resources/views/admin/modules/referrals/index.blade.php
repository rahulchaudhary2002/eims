...
<div class="space-y-5">
    <x-admin.page-header
        title="Referrals"
        subtitle="Manage referral records linked to applications."
        :breadcrumbs="[
    ...
    <div class="eims-card p-4">
        <form method="GET" action="{{ route('admin.referrals.index') }}" class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-3 items-end">
            <div>
            ...
            <div>
                <label class="form-label text-xs">Referred By</label>
                <select name="referred_by" class="form-control">
                    <option value="">All Users</option>
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ request('referred_by') == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            <div>
                <label class="form-label text-xs">Referred From</label>
            ...
            <div class="flex gap-2 md:col-span-3 xl:col-span-4">
                ...
                <a href="{{ route('admin.referrals.index') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="eims-table">
                ...
                <tbody>
                    @forelse($referrals as $referral)
                        <tr>
                            <td>
                                <a href="{{ route('admin.referrals.show', $referral) }}" class="font-semibold text-blue-600 hover:underline font-mono text-sm">
                                    {{ $referral->referral_number }}
                                </a>
                            </td>
                            <td>
                                @if($referral->application)
                                    <a href="{{ route('admin.applications.show', $referral->application) }}" class="text-blue-600 hover:underline text-sm">
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    ...
                                    <form action="{{ route('admin.referrals.destroy', $referral) }}" method="POST" onsubmit="return confirm('Delete this referral?')">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="text-center text-slate-400 py-10">No referrals found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
