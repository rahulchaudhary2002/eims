...
<div class="bg-white rounded-lg shadow-lg border border-gray-200">
    <div class="p-6 flex justify-between items-center mb-6 bg-white bg-opacity-80 backdrop-blur-sm rounded-t-lg">
        <div>
            <h1 class="text-3xl font-bold text-gray-800">🏷️ Edit Institution Category</h1>
            <p class="text-gray-600 mt-1">Update institution category details</p>
        </div>
        <a href="{{ route('admin.institution-category.index') }}" class="bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 text-white font-bold py-2 px-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-200 transform hover:-translate-y-0.5 flex items-center">
            <x-lucide-arrow-left class="w-5 h-5 mr-2" />
            Back to Institution Categories
        </a>
    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="name" class="block text-sm font-medium text-gray-700 mb-2 flex items-center">
                            <x-lucide-tags class="w-5 h-5 mr-2 text-blue-500" />
                            Institution Category Name *
                        </label>
                        <input type="text" name="name" id="name" value="{{ old('name', $institutionCategory->name) }}"
                            ...
                            placeholder="Enter institution category name" required>
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $institutionCategory->slug) }}"
