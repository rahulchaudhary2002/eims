...
@section('title', 'Referral Agreements')
@section('page-title', 'Referral Agreements')

@section('content')
<div class="space-y-5">
    <x-admin.page-header
        title="Referral Agreements"
        subtitle="Manage referral commission agreements with institutions."
        :breadcrumbs="[
            ['label' => 'Dashboard', 'route' => 'admin.dashboard'],
            ['label' => 'Referral Agreements'],
        ]">
        <x-slot:actions>
            <a href="{{ route('admin.referral-agreements.create') }}" class="btn btn-primary">
    ...
    <div class="eims-card p-4">
        <form method="GET" action="{{ route('admin.referral-agreements.index') }}" class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-3 items-end">
            <div>
            ...
            <div class="flex gap-2 md:col-span-3 xl:col-span-4">
                ...
                <a href="{{ route('admin.referral-agreements.index') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="eims-card overflow-hidden">
            ...
            <table class="eims-table">
                ...
                <tbody>
                    @forelse($agreements as $agreement)
                        <tr>
                            <td>
                                <a href="{{ route('admin.referral-agreements.show', $agreement) }}" class="font-semibold text-blue-600 hover:underline">
                            ...
                            <td><span class="badge">{{ $statuses[$agreement->status] ?? $agreement->status }}</span></td>
                            <td>
                                <div class="flex items-center gap-1">
                                    ...
                                    <form action="{{ route('admin.referral-agreements.destroy', $agreement) }}" method="POST" onsubmit="return confirm('Delete this referral agreement?')">
                                        @csrf @method('DELETE')
        ...
        @if($agreements->hasPages())
            <div class="px-4 py-3 border-t border-slate-100">{{ $agreements->links() }}</div>
        @endif
    </div>
</div>
@endsection
