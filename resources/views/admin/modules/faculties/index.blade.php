...
<div class="space-y-5">
    ...
    <x-admin.page-header title="Faculties" subtitle="Manage academic faculties"
        :breadcrumbs="[['label'=>'Dashboard','route'=>'admin.dashboard'],['label'=>'Faculties']]">
        <x-slot:actions>
    ...
    <div class="eims-card p-4">
        <form method="GET" action="{{ route('admin.faculties.index') }}" class="flex flex-wrap gap-3 items-end">
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('admin.faculties.index') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="eims-table w-full">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-slate-400 text-sm">{{ $faculties->firstItem() + $loop->index }}</td>
                        <td class="font-medium text-slate-800">{{ $faculty->name }}</td>
                        <td class="text-sm font-mono text-slate-500">{{ $faculty->slug }}</td>
                        <td>
                        ...
                        <td class="text-sm text-slate-500">{{ $faculty->created_at->format('d M Y') }}</td>
                        <td>
                            <div class="flex items-center justify-center gap-1">
                                <a href="{{ route('admin.faculties.show', $faculty) }}" class="btn-icon btn-icon-view" title="View">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                </a>
                                <a href="{{ route('admin.faculties.edit', $faculty) }}" class="btn-icon btn-icon-edit" title="Edit">
