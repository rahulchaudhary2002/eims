...
<div class="space-y-5">
            ...
            <table class="eims-table w-full">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ \App\Models\UserInstitution::ROLES[$usr->pivot->role] ?? $usr->pivot->role ?? '-' }}</td>
    ...
    <div class="eims-card overflow-hidden">
        <div class="flex items-center gap-3 px-6 py-4 border-b border-slate-100">
            ...
            <span class="ml-1 text-xs text-slate-400">{{ $institution->courses->count() }} course(s)</span>
            <div class="ml-auto">
                <a href="{{ route('admin.institution-courses.create', ['institution_id' => $institution->id]) }}" class="btn btn-primary text-xs py-1.5">Add Course</a>
            </div>
        </div>
        @if($institution->courses->isEmpty())
            <p class="text-sm text-slate-400 text-center py-8">No courses added yet.</p>
        @else
            <div class="overflow-x-auto">
                <table class="eims-table">
                    ...
                    <tbody>
                        @foreach($institution->courses->take(5) as $course)
                            <tr>
                                <td class="text-sm font-medium text-slate-800">{{ $course->title }}</td>
                                <td class="text-sm text-slate-600">{{ $course->fee ? number_format($course->fee, 2) : '-' }}</td>
                                <td class="text-sm text-slate-600">{{ $course->duration_hours ? $course->duration_hours . ' hrs' : '-' }}</td>
                                <td>
                                    @if($course->is_active)
                                ...
                                <td>
                                    <div class="flex items-center gap-1">
                                        <a href="{{ route('admin.institution-courses.show', $course) }}" class="btn-icon btn-icon-view" title="View">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                        </a>
                                        <a href="{{ route('admin.institution-courses.edit', $course) }}" class="btn-icon btn-icon-edit" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/></svg>
                                        </a>
                                    </div>
                                </td>
            ...
            @if($institution->courses->count() > 5)
                <div class="px-6 py-3 border-t border-slate-100">
                    <a href="{{ route('admin.institution-courses.index', ['institution_id' => $institution->id]) }}" class="text-sm text-blue-600 hover:text-blue-800 hover:underline">View all {{ $institution->courses->count() }} courses &rarr;</a>
                </div>
            @endif
        @endif
    </div>
    {{-- Certifications (all institution types) --}}
    <div class="eims-card overflow-hidden">
        <div class="flex items-center gap-3 px-6 py-4 border-b border-slate-100">
            <div class="p-2 bg-amber-50 rounded-lg">
            ...
            <span class="ml-1 text-xs text-slate-400">{{ $institution->certifications->count() }} certification(s)</span>
            <div class="ml-auto">
                <a href="{{ route('admin.institution-certifications.create', ['institution_id' => $institution->id]) }}" class="btn btn-primary text-xs py-1.5">Add Certification</a>
            </div>
        </div>
        @if($institution->certifications->isEmpty())
            <p class="text-sm text-slate-400 text-center py-8">No certifications added yet.</p>
        @else
            <div class="overflow-x-auto">
                <table class="eims-table">
                    <thead>
                    ...
                    <tbody>
                        @foreach($institution->certifications->take(5) as $cert)
                            <tr>
                                <td class="text-sm font-medium text-slate-800">{{ $cert->title }}</td>
                                <td class="text-sm text-slate-600">{{ $cert->fee ? number_format($cert->fee, 2) : '-' }}</td>
                                <td class="text-sm text-slate-600">{{ $cert->duration_hours ? $cert->duration_hours . ' hrs' : '-' }}</td>
                                <td>
                                    @if($cert->is_active)
                                        <span class="badge badge-green text-xs">Active</span>
                                    @else
                                ...
                                <td>
                                    <div class="flex items-center gap-1">
                                        <a href="{{ route('admin.institution-certifications.show', $cert) }}" class="btn-icon btn-icon-view" title="View">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 010-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                                        </a>
                                        <a href="{{ route('admin.institution-certifications.edit', $cert) }}" class="btn-icon btn-icon-edit" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/></svg>
                                        </a>
                                    </div>
                                </td>
            ...
            @if($institution->certifications->count() > 5)
                <div class="px-6 py-3 border-t border-slate-100">
                    <a href="{{ route('admin.institution-certifications.index', ['institution_id' => $institution->id]) }}" class="text-sm text-blue-600 hover:text-blue-800 hover:underline">View all {{ $institution->certifications->count() }} certifications &rarr;</a>
                </div>
            @endif
        @endif
    </div>
