...
<div class="space-y-5">
    ...
    <x-admin.page-header title="Create Vendor" subtitle="Add a new service provider"
        :breadcrumbs="[['label'=>'Dashboard','route'=>'admin.dashboard'],['label'=>'Vendors','route' => 'admin.vendor.index'],['label'=>'Create']]">
        <x-slot:actions>
    ...
    <x-admin.form-card title="Vendor Details">
        <form action="{{ route('admin.vendor.store') }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <x-admin.form-input name="name" label="Vendor Name" :value="old('name')" required />
                <x-admin.form-input name="email" label="Email Address" type="email" :value="old('email')" required />
                <x-admin.form-input name="phone" label="Phone Number" :value="old('phone')" />
                ...
                <div class="md:col-span-2">
                    ...
                    <select name="institutions[]" id="institutions" multiple
                        ...
                        <option value="{{ $institution->id }}"
                            {{ (collect(old('institutions'))->contains($institution->id)) ? 'selected' : '' }}>
                ...
                <x-admin.form-input name="password" label="Password" type="password" required />
                <x-admin.form-input name="password_confirmation" label="Confirm Password" type="password" required />
            </div>
