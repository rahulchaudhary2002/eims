...
<div class="space-y-5" x-data="{ tab: '{{ session('tab', 'overview') }}' }">
    ...
    <div class="eims-card p-6">
        <div class="flex flex-wrap items-center gap-5">
            ...
            <div class="flex-1 min-w-0">
                ...
                <p class="text-xs text-slate-400 mt-0.5">Registered {{ $student->created_at->diffForHumans() }}</p>
            </div>
            {{-- Quick stats --}}
            <div class="flex flex-wrap gap-4">
                @php
                    $statItems = [
                        ['Applications', $student->applications->count(), 'bg-blue-50 text-blue-700'],
                        ['Academic Records', $student->academicRecords->count(), 'bg-violet-50 text-violet-700'],
                        ['Documents', $student->documents->count(), 'bg-amber-50 text-amber-700'],
                        ['Sessions', $student->counselingSessions->count(), 'bg-emerald-50 text-emerald-700'],
                    ];
                @endphp
                @foreach($statItems as [$label, $count, $cls])
                <div class="text-center px-4 py-2 rounded-xl {{ $cls }}">
    ...
    @php
        $tabs = [
            ...
            ['key' => 'academic',      'label' => 'Academic',      'count' => $student->academicRecords->count()],
            ['key' => 'documents',     'label' => 'Documents',     'count' => $student->documents->count()],
            ['key' => 'applications',  'label' => 'Applications',  'count' => $student->applications->count() + $student->scholarshipApplications->count()],
            ['key' => 'engagement',    'label' => 'Engagement',    'count' => $student->favoriteInstitutions->count() + $student->compareItems->count() + $student->recommendations->count() + $student->followedInstitutions->count()],
            ['key' => 'sessions',      'label' => 'Sessions',      'count' => $student->counselingSessions->count()],
            ['key' => 'communication', 'label' => 'Communication', 'count' => $student->reviews->count() + $student->conversations->count()],
        ];
    @endphp
    <div class="eims-card overflow-x-auto">
        <nav class="flex gap-1 p-1.5 min-w-max">
            ...
            <button
                ...
                <span
                    :class="tab === '{{ $t['key'] }}' ? 'bg-primary-100 text-primary-700' : 'bg-slate-200 text-slate-500'"
                    class="inline-flex items-center justify-center w-5 h-5 rounded-full text-xs font-bold transition-colors">
                    {{ $t['count'] }}
                </span>
                @endif
            </button>
    ...
    <div x-show="tab === 'profile'" x-cloak>
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                ...
                @if($student->profile)
                    <div class="flex gap-2">
                        <a href="{{ route('admin.student-profiles.show', $student->profile) }}" class="btn btn-secondary text-sm">View Full Profile</a>
                        <a href="{{ route('admin.student-profiles.edit', $student->profile) }}" class="btn btn-primary text-sm">Edit Profile</a>
                    </div>
                @else
                ...
                <div class="space-y-2">
                    ...
                    @if($student->profile->province || $student->profile->district || $student->profile->city)
                        <p class="text-slate-800">
                            {{ implode(', ', array_filter([$student->profile->city, $student->profile->district, $student->profile->province])) }}
                        </p>
            ...
            <div class="px-6 py-10 text-center text-slate-400">
                ...
                <a href="{{ route('admin.student-profiles.create', ['student_id' => $student->id]) }}" class="btn btn-primary text-sm">Create Profile Now</a>
            </div>
            @endif
        </div>
    </div>
        ...
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                ...
                <div class="flex gap-2">
                    @if($student->academicRecords->isNotEmpty())
                        <a href="{{ route('admin.student-academic-records.index', ['student_id' => $student->id]) }}" class="btn btn-secondary text-sm">View All</a>
                    @endif
                    <a href="{{ route('admin.student-academic-records.create', ['student_id' => $student->id]) }}" class="btn btn-primary text-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15"/></svg>
                        Add Record
                    </a>
                </div>
            ...
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    ...
                    <tbody>
                        @foreach($student->academicRecords->sortByDesc('passed_year') as $record)
                        <tr>
                            <td><span class="badge badge-blue">{{ \App\Models\StudentAcademicRecord::LEVELS[$record->level] ?? $record->level }}</span></td>
                            <td class="text-sm">{{ $record->institution_name ?? '-' }}</td>
                            <td class="text-sm">{{ \App\Models\StudentAcademicRecord::BOARDS[$record->board] ?? ($record->board ?? '-') }}</td>
                            <td class="text-sm">{{ $record->faculty ?? '-' }}</td>
                            <td class="text-sm">{{ $record->passed_year ?? '-' }}</td>
                            <td class="text-sm">
                                @if($record->gpa !== null) {{ number_format($record->gpa, 2) }} GPA @endif
                                @if($record->percentage !== null)
                                    @if($record->gpa !== null) / @endif
                                    {{ number_format($record->percentage, 2) }}%
                                @endif
                                @if($record->gpa === null && $record->percentage === null)<span class="text-slate-300">-</span>@endif
                            </td>
                            <td class="text-center text-sm">
                                @php $docCount = (int)($record->transcript_file ? 1 : 0) + (int)($record->character_certificate_file ? 1 : 0); @endphp
                                @if($docCount)
                            ...
                            <td class="text-center">
                                <div class="flex justify-center gap-1">
                                    <a href="{{ route('admin.student-academic-records.show', $record) }}" class="btn-icon btn-icon-view" title="View">
            ...
            <div class="px-6 py-10 text-center text-slate-400">
                ...
                <a href="{{ route('admin.student-academic-records.create', ['student_id' => $student->id]) }}" class="btn btn-primary text-sm">Add First Record</a>
            </div>
            @endif
        </div>
    </div>
    ...
    <div x-show="tab === 'documents'" x-cloak>
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="font-semibold text-slate-800">Student Documents</h3>
                    <p class="text-xs text-slate-500 mt-0.5">Uploaded identity &amp; supporting documents</p>
                </div>
                <div class="flex items-center gap-2">
                    @if($student->documents->count() > 0)
                    <a href="{{ route('admin.student-documents.index', ['student_id' => $student->id]) }}" class="btn btn-secondary text-sm">View All</a>
                    @endif
                    <a href="{{ route('admin.student-documents.create', ['student_id' => $student->id]) }}" class="btn btn-primary text-sm">Upload Document</a>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    ...
                    <tbody>
                        @foreach($student->documents->sortByDesc('created_at') as $doc)
                        <tr>
                            <td><span class="badge badge-blue">{{ \App\Models\StudentDocument::DOCUMENT_TYPES[$doc->document_type] ?? $doc->document_type }}</span></td>
                            <td class="text-sm font-medium text-slate-800">{{ $doc->title }}</td>
                            <td>
                                <a href="{{ storage_url($doc->file_path) }}" target="_blank" class="inline-flex items-center gap-1 text-sm text-blue-600 hover:underline">
                            ...
                            <td>
                                @if($doc->status === 'active') <span class="badge badge-green">Active</span>
                                @elseif($doc->status === 'expired') <span class="badge badge-red">Expired</span>
                                @else <span class="badge badge-yellow">Inactive</span>
                                @endif
                            </td>
            ...
            <div class="px-6 py-10 text-center text-slate-400">
                ...
                <a href="{{ route('admin.student-documents.create', ['student_id' => $student->id]) }}" class="btn btn-primary text-sm">Upload First Document</a>
            </div>
            @endif
        </div>
    </div>
    ...
    {{-- ══════════════════════════════════════════════
         TAB: APPLICATIONS
    ══════════════════════════════════════════════ --}}
    <div x-show="tab === 'applications'" x-cloak class="space-y-5">

        {{-- Applications --}}
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="font-semibold text-slate-800">Applications</h3>
        ...
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="font-semibold text-slate-800">Scholarship Applications</h3>
            ...
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    ...
                    <tbody>
                        @foreach($student->scholarshipApplications as $sa)
                        <tr>
                            <td class="font-medium text-sm">{{ $sa->scholarship->title ?? '-' }}</td>
                            <td class="text-sm text-slate-500">{{ $sa->scholarship?->institution?->name ?? '-' }}</td>
                            <td class="font-mono text-sm">{{ $sa->scholarship_approved_amount !== null ? number_format((float) $sa->scholarship_approved_amount, 2) : '-' }}</td>
                            <td><span class="badge">{{ \App\Models\Application::SCHOLARSHIP_STATUSES[$sa->scholarship_status] ?? ($sa->scholarship_status ?? '-') }}</span></td>
                            <td>
                                <div class="flex justify-center gap-1">
            ...
            @else
            <div class="px-6 py-10 text-center text-slate-400">
                <p class="text-sm">No scholarship applications yet.</p>
            </div>
            @endif
        </div>
        ...
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="font-semibold text-slate-800">Scholarship Cashbacks</h3>
            ...
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    ...
                    <tbody>
                        @foreach($student->scholarshipCashbacks as $cashback)
                        <tr>
                            <td class="font-mono text-xs text-slate-500">
                                @if($cashback->commissionInvoice)
                                    <a href="{{ route('admin.commission-invoices.show', $cashback->commissionInvoice) }}" class="text-blue-600 hover:underline">{{ $cashback->commissionInvoice->invoice_number }}</a>
                                @else
                                    -
                                @endif
                            </td>
    ...
    {{-- ══════════════════════════════════════════════
         TAB: ENGAGEMENT
    ══════════════════════════════════════════════ --}}
    <div x-show="tab === 'engagement'" x-cloak class="space-y-5">

        {{-- Favorite Institutions --}}
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="font-semibold text-slate-800">Favorite Institutions</h3>
            ...
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    <thead><tr><th>Institution</th><th>Added At</th><th class="text-center">Actions</th></tr></thead>
                    <tbody>
                        @foreach($student->favoriteInstitutions as $fav)
                        <tr>
                            <td>
                                @if($fav->institution)
                                    <a href="{{ route('admin.institutions.show', $fav->institution) }}" class="font-medium text-blue-600 hover:underline text-sm">{{ $fav->institution->name }}</a>
                                @else
            ...
            @else
            <div class="px-6 py-8 text-center text-slate-400 text-sm">No favorite institutions yet.</div>
            @endif
        </div>
        ...
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="font-semibold text-slate-800">Followed Institutions</h3>
            ...
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    <thead><tr><th>Institution</th><th>Followed At</th><th class="text-center">Actions</th></tr></thead>
                    <tbody>
                        @foreach($student->followedInstitutions as $follow)
                        <tr>
                            <td>
                                @if($follow->institution)
                                    <a href="{{ route('admin.institutions.show', $follow->institution) }}" class="font-medium text-blue-600 hover:underline text-sm">{{ $follow->institution->name }}</a>
                                @else
                                    <span class="text-slate-400">-</span>
                                @endif
                            </td>
            ...
            @else
            <div class="px-6 py-8 text-center text-slate-400 text-sm">No followed institutions yet.</div>
            @endif
        </div>
        ...
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="font-semibold text-slate-800">Compare Items</h3>
            ...
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    <thead><tr><th>Institution</th><th>Program</th><th>Added At</th><th class="text-center">Actions</th></tr></thead>
                    <tbody>
                        @foreach($student->compareItems as $item)
                        <tr>
                            <td>
                                @if($item->institution)
                                    <a href="{{ route('admin.institutions.show', $item->institution) }}" class="font-medium text-blue-600 hover:underline text-sm">{{ $item->institution->name }}</a>
                                @else <span class="text-slate-400">-</span>
                                @endif
                            </td>
                            <td class="text-sm text-slate-600">{{ $item->institutionProgram?->title ?: ($item->institutionProgram?->program?->name ?? '-') }}</td>
            ...
            @else
            <div class="px-6 py-8 text-center text-slate-400 text-sm">No compare items yet.</div>
            @endif
        </div>
        ...
        <div class="eims-card overflow-hidden">
            ...
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    <thead><tr><th>Institution</th><th>Program</th><th>Score</th><th>Viewed</th><th class="text-center">Actions</th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-sm text-slate-600">{{ $rec->institutionProgram?->title ?: ($rec->institutionProgram?->program?->name ?? '-') }}</td>
                            <td>
                                @if($rec->score !== null)
                            ...
                            <td>
                                @if($rec->is_viewed) <span class="badge badge-green text-xs">Viewed</span>
                                @else <span class="badge text-xs">Not Viewed</span>
                                @endif
                            </td>
                            <td>
                                <div class="flex justify-center gap-1">
            ...
            @else
            <div class="px-6 py-8 text-center text-slate-400 text-sm">No recommendations yet.</div>
            @endif
        </div>
    </div>
    ...
    <div x-show="tab === 'sessions'" x-cloak>
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="font-semibold text-slate-800">Counseling Sessions</h3>
            ...
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    ...
                    <tbody>
                        @foreach($student->counselingSessions as $cs)
                        <tr>
    ...
    {{-- ══════════════════════════════════════════════
         TAB: COMMUNICATION
    ══════════════════════════════════════════════ --}}
    <div x-show="tab === 'communication'" x-cloak class="space-y-5">

        {{-- Reviews --}}
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    <h3 class="font-semibold text-slate-800">Institution Reviews</h3>
                    <p class="text-xs text-slate-500 mt-0.5">{{ $student->reviews->count() }} review{{ $student->reviews->count() !== 1 ? 's' : '' }} submitted</p>
                </div>
                <a href="{{ route('admin.institution-reviews.create', ['student_id' => $student->id]) }}" class="btn btn-primary text-sm">Add Review</a>
            </div>
            @if($student->reviews->isNotEmpty())
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    <thead><tr><th>Institution</th><th>Rating</th><th>Review</th><th>Approved</th><th>Date</th><th class="text-center">Actions</th></tr></thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="font-semibold text-amber-500">{{ $review->rating }}</span> <span class="text-slate-400 text-xs">/ 5</span></td>
                            <td class="text-sm max-w-xs truncate text-slate-600">{{ $review->review ? \Illuminate\Support\Str::limit($review->review, 50) : '-' }}</td>
                            <td>
                                @if($review->is_approved) <span class="badge badge-success">Approved</span>
            ...
            @else
            <div class="px-6 py-8 text-center text-slate-400 text-sm">No reviews submitted yet.</div>
            @endif
        </div>
        ...
        <div class="eims-card overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-slate-500 mt-0.5">{{ $student->conversations->count() }} conversation{{ $student->conversations->count() !== 1 ? 's' : '' }}</p>
                </div>
                <a href="{{ route('admin.conversations.create', ['student_id' => $student->id]) }}" class="btn btn-primary text-sm">New Conversation</a>
            </div>
            @if($student->conversations->isNotEmpty())
            <div class="overflow-x-auto">
                <table class="eims-table w-full">
                    <thead><tr><th>Institution</th><th>Type</th><th>Started</th><th class="text-center">Actions</th></tr></thead>
                    <tbody>
                        @foreach($student->conversations as $conv)
                        <tr>
                            <td class="text-sm">
                                @if($conv->institution)
                            ...
                            <td><span class="badge">{{ \App\Models\Conversation::TYPES[$conv->type] ?? $conv->type }}</span></td>
            ...
            @else
            <div class="px-6 py-8 text-center text-slate-400 text-sm">No conversations yet.</div>
            @endif
        </div>
    </div>
