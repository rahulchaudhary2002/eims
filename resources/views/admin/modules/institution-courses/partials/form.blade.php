    ...
    <select id="institution_id" name="institution_id" class="form-control @error('institution_id') is-invalid @enderror">
        ...
        @foreach($institutions as $institution)
            <option value="{{ $institution->id }}"
                {{ old('institution_id', $institutionCourse->institution_id ?? $selectedInstitutionId ?? '') == $institution->id ? 'selected' : '' }}>
                {{ $institution->name }}
            </option>
        @endforeach
    </select>
...
<div>
    ...
    <input type="text" id="title" name="title"
        value="{{ old('title', $institutionCourse->title ?? '') }}"
...
<div>
    ...
    <input type="number" id="fee" name="fee" step="0.01" min="0"
        value="{{ old('fee', $institutionCourse->fee ?? '') }}"
...
<div>
    ...
    <input type="number" id="duration_hours" name="duration_hours" min="1"
        value="{{ old('duration_hours', $institutionCourse->duration_hours ?? '') }}"
...
<div class="flex items-center gap-3 pt-2">
    ...
    <input type="checkbox" id="is_active" name="is_active" value="1"
        {{ old('is_active', $institutionCourse->is_active ?? true) ? 'checked' : '' }}
...
<div class="md:col-span-2">
    ...
    <textarea id="description" name="description" rows="6"
        class="form-control ckeditor @error('description') is-invalid @enderror">{{ old('description', $institutionCourse->description ?? '') }}</textarea>
