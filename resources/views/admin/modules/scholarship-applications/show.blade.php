...
<div class="space-y-5">
    <x-admin.page-header
        ...
        <x-slot:actions>
            <a href="{{ route('admin.scholarship-applications.edit', $scholarshipApplication) }}" class="btn btn-primary">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="lg:col-span-2">
            <div class="eims-card p-6">
                ...
                <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-sm">
                    ...
                    <div>
                        <dt class="text-xs font-medium text-slate-500 uppercase tracking-wide">Linked Application</dt>
                        <dd class="mt-1">
                            @if($scholarshipApplication->application)
                                <a href="{{ route('admin.applications.show', $scholarshipApplication->application) }}" class="text-blue-600 hover:underline font-mono text-sm">
                                    {{ $scholarshipApplication->application->application_number }}
                                </a>
                                <div class="text-xs text-slate-400 mt-0.5">
                                    {{ $scholarshipApplication->application->institutionProgram?->title
                    ...
                    @if($scholarshipApplication->remarks)
                        <div class="sm:col-span-2">
                            ...
                            <dd class="mt-1 text-slate-700 whitespace-pre-line">{{ $scholarshipApplication->remarks }}</dd>
                        </div>
                    @endif
                </dl>
            </div>
        ...
        <div class="space-y-5">
            <div class="eims-card p-5">
                ...
                <span class="badge text-sm">{{ \App\Models\ScholarshipApplication::STATUSES[$scholarshipApplication->status] ?? $scholarshipApplication->status }}</span>
                ...
                <form action="{{ route('admin.scholarship-applications.update-status', $scholarshipApplication) }}" method="POST" class="mt-4 space-y-2">
                    ...
                    <select name="status" class="form-control text-sm">
                        @foreach(\App\Models\ScholarshipApplication::STATUSES as $value => $label)
                            <option value="{{ $value }}" {{ $scholarshipApplication->status === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div class="eims-card p-5">
                ...
                <div class="space-y-2">
                    <a href="{{ route('admin.scholarship-applications.edit', $scholarshipApplication) }}" class="btn btn-secondary w-full text-sm">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/></svg>
                        Edit Application
                    </a>
                    <form action="{{ route('admin.scholarship-applications.destroy', $scholarshipApplication) }}" method="POST" onsubmit="return confirm('Delete this scholarship application? This cannot be undone.')">
                        @csrf @method('DELETE')
