...
<div class="space-y-5">
    ...
    <div class="eims-card p-4">
        <form method="GET" action="{{ route('admin.scholarship-applications.index') }}" class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-3 items-end">
            ...
            <div>
                <label class="form-label text-xs">Scholarship Status</label>
                <select name="scholarship_status" class="form-control">
                    <option value="">All Statuses</option>
                    @foreach($statuses as $value => $label)
                        <option value="{{ $value }}" {{ request('scholarship_status') === $value ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
                    ...
                    @forelse($scholarshipApplications as $sa)
                        <tr>
                            ...
                            <td class="font-mono text-xs text-slate-500">
                                <a href="{{ route('admin.applications.show', $sa) }}" class="text-blue-600 hover:underline">
                                    {{ $sa->application_number }}
                                </a>
                            </td>
                            <td class="font-mono text-sm">
                                {{ $sa->scholarship_approved_amount !== null ? number_format((float) $sa->scholarship_approved_amount, 2) : '-' }}
                            </td>
                            <td><span class="badge">{{ $statuses[$sa->scholarship_status] ?? ($sa->scholarship_status ?? '-') }}</span></td>
                            <td>
                                <div class="flex items-center gap-1">
                                    ...
                                    <a href="{{ route('admin.applications.edit', $sa) }}" class="btn-icon btn-icon-edit" title="Edit Application">
