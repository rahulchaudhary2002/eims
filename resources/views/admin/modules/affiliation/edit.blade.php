...
@section('page-title', 'Edit Affiliation')
@section('content')
<div class="space-y-5">
    <x-admin.page-header title="Edit Affiliation" subtitle="Update details for: {{ $affiliation->name }}"
        :breadcrumbs="[['label'=>'Dashboard','route'=>'admin.dashboard'],['label'=>'Affiliations','route'=>'admin.affiliation.index'],['label'=>'Edit']]">
        <x-slot:actions>
            <a href="{{ route('admin.affiliation.index') }}" class="btn btn-secondary">
    ...
    @endif
    <x-admin.form-card title="Affiliation Details">
        <form action="{{ route('admin.affiliation.update', $affiliation) }}" method="POST">
            @csrf @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <x-admin.form-input name="name" label="Affiliation Name" :value="old('name', $affiliation->name)" required />
                <x-admin.form-input name="code" label="Code" :value="old('code', $affiliation->code)" />
                <div class="md:col-span-2">
                    <x-admin.form-textarea name="description" label="Description" :value="old('description', $affiliation->description)" :rows="3" />
                </div>
                <div class="md:col-span-2 flex items-center gap-3 pt-1">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $affiliation->is_active) ? 'checked' : '' }} class="w-4 h-4 text-primary-600 border-slate-300 rounded focus:ring-primary-500">
            ...
            <div class="flex items-center justify-end gap-3 mt-6 pt-5 border-t border-slate-100">
                <a href="{{ route('admin.affiliation.index') }}" class="btn btn-secondary">Cancel</a>
            ...
            <form action="{{ route('admin.affiliation.update', $affiliation) }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $affiliation->name) }}"
                    ...
                    <div>
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code', $affiliation->code) }}"
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="4"
                            ...
                            placeholder="Enter affiliation description">{{ old('description', $affiliation->description) }}</textarea>
                        @error('description')
                        <p class="text-red-500 text-sm mt-1 flex items-center">
                            <x-lucide-alert-circle class="w-4 h-4 mr-1" />
                            {{ $message }}
