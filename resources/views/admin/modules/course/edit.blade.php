...
<div class="space-y-5">
    <x-admin.page-header title="Edit Course" subtitle="Update details for: {{ $course->name }}"
        :breadcrumbs="[['label'=>'Dashboard','route'=>'admin.dashboard'],['label'=>'Courses','route' => 'admin.course.index'],['label'=>'Edit']]">
        <x-slot:actions>
    ...
    <x-admin.form-card title="Course Details">
        <form action="{{ route('admin.course.update', $course) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <x-admin.form-input name="name" label="Course Name" :value="old('name', $course->name)" required />
                <x-admin.form-input name="code" label="Course Code" :value="old('code', $course->code)" required />
                <div class="md:col-span-2">
                    <label class="form-label" for="program_ids">Programs <span class="text-danger-600">*</span></label>
                    <select name="program_ids[]" id="program_ids" multiple class="form-control @error('program_ids') border-red-400 @enderror" required>
                        @foreach($programs as $program)
                        <option value="{{ $program->id }}" {{ in_array($program->id, $selectedProgramIds) ? 'selected' : '' }}>{{ $program->name }}</option>
                        @endforeach
                    </select>
                    @error('program_ids')<p class="form-error">{{ $message }}</p>@enderror
                </div>
                <div class="md:col-span-2">
                    <x-admin.form-textarea name="description" label="Description" :value="old('description', $course->description)" :rows="3" />
                </div>
                <div class="md:col-span-2 flex items-center gap-3 pt-1">
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $course->is_active) ? 'checked' : '' }} class="w-4 h-4 text-primary-600 border-slate-300 rounded focus:ring-primary-500">
                    ...
                    <div class="bg-slate-50 p-4 rounded-input border border-slate-200 mb-4 flex flex-col gap-3">
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex gap-1.5">
                                <button type="button" @click="moveUp(index)" class="btn-icon" title="Move Up">&uarr;</button>
                                <button type="button" @click="moveDown(index)" class="btn-icon" title="Move Down">&darr;</button>
                        ...
                        <input type="text" :name="'sections['+index+'][title]'" x-model="section.title" placeholder="Section Title" class="form-control">
                        <textarea :id="'content-'+index" :name="'sections['+index+'][content]'" x-model="section.content" placeholder="Section Content" class="form-control"></textarea>
                    </div>
                </template>
