...
@section('title', 'Student Compare Items')
@section('page-title', 'Student Compare Items')

@section('content')
<div class="space-y-5">
    <x-admin.page-header
        title="Compare Items"
        subtitle="Institutions and programs added to student comparison lists."
        :breadcrumbs="[
            ['label' => 'Dashboard', 'route' => 'admin.dashboard'],
            ['label' => 'Compare Items'],
        ]">
        <x-slot:actions>
            <a href="{{ route('admin.student-compare-items.create') }}" class="btn btn-primary">
    ...
    <div class="eims-card p-4">
        <form method="GET" action="{{ route('admin.student-compare-items.index') }}" class="grid grid-cols-1 md:grid-cols-3 xl:grid-cols-4 gap-3 items-end">
            <div>
            ...
            <div class="flex gap-2 md:col-span-3 xl:col-span-4">
                ...
                <a href="{{ route('admin.student-compare-items.index') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="eims-card overflow-hidden">
        <div class="overflow-x-auto">
            <table class="eims-table">
                ...
                <tbody>
                    @forelse($compareItems as $item)
                        <tr>
                            <td>
                                @if($item->student)
                                    <a href="{{ route('admin.students.show', $item->student) }}" class="font-medium text-blue-600 hover:underline text-sm">
                                        {{ $item->student->name }}
                                    </a>
                            ...
                            <td>
                                <div class="flex items-center gap-1">
                                    ...
                                    <a href="{{ route('admin.student-compare-items.edit', $item) }}" class="btn-icon btn-icon-edit" title="Edit">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-slate-400 py-10">No compare items found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        @if($compareItems->hasPages())
            <div class="px-4 py-3 border-t border-slate-100">{{ $compareItems->links() }}</div>
        @endif
    </div>
</div>
@endsection
