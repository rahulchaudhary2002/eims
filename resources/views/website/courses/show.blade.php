...
@section('meta-title', $course->title . ' - ' . config('app.name'))
@section('meta-description', $course->description ? Str::limit(strip_tags($course->description), 160) : 'Learn about ' . $course->title . ' at ' . ($course->institution->name ?? '') . '. View fee, duration, and apply.')
...
<section class="bg-gradient-to-br from-[#2c5aa0] to-[#1a365d] pt-[150px] pb-28 text-white">
    <div class="container max-w-7xl mx-auto px-4">
        @include('website.partials.breadcrumb', [
            'variant' => 'dark',
            'breadcrumbs' => [
                ['label' => 'Courses', 'url' => route('website.courses.index')],
                ['label' => $course->title],
            ],
        ])
        ...
        <div class="grid lg:grid-cols-[minmax(0,1fr)_340px] gap-10 items-center mt-12">
            <div>
                <span class="inline-flex items-center gap-2 bg-white/10 border border-white/20 rounded-full px-4 py-2 text-sm font-semibold mb-5">
                    <i class="fas fa-book-open text-teal-300"></i> Course
                </span>

                <h1 class="text-[2.6rem] md:text-[3.2rem] font-bold leading-[1.15] mb-5">{{ $course->title }}</h1>

                @if ($course->description)
                    <p class="text-[1.05rem] text-white/85 leading-relaxed max-w-2xl">
                        {{ Str::limit(strip_tags($course->description), 220) }}
                    </p>
                @endif
            ...
            <div class="bg-white/10 border border-white/20 rounded-xl p-6 shadow-[0_15px_40px_rgba(0,0,0,0.18)]">
                <div class="text-center mb-5">
                    <div class="w-14 h-14 rounded-full bg-teal-400/20 border border-teal-400/40 flex items-center justify-center mx-auto mb-3">
                        <i class="fas fa-book-open text-2xl text-teal-300"></i>
                    </div>
                    <p class="text-sm text-white/70">Professional Course</p>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    @if ($course->fee)
                        <div class="rounded-xl bg-white/10 border border-white/15 p-4">
                            <p class="text-xs text-white/65 mb-1">Course Fee</p>
                            <p class="font-bold text-lg">NPR {{ number_format($course->fee) }}</p>
                        </div>
                    @endif
                    @if ($course->duration_hours)
                        <div class="rounded-xl bg-white/10 border border-white/15 p-4">
                            <p class="text-xs text-white/65 mb-1">Duration</p>
                            <p class="font-bold text-lg">{{ $course->duration_hours }} hrs</p>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div class="grid lg:grid-cols-[minmax(0,1fr)_320px] gap-8 items-start">
            ...
            <div class="space-y-8">
                ...
                <div class="bg-white rounded-xl shadow-[0_5px_15px_rgba(0,0,0,0.08)] border border-gray-200 p-6 md:p-8">
                    <h2 class="relative inline-block text-[2rem] font-bold text-[#2c5aa0] mb-6 after:content-[''] after:absolute after:left-0 after:-bottom-2 after:w-[60px] after:h-[3px] after:bg-[#4299e1]">Course Details</h2>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mt-4">
                        @if ($course->fee)
                            <div class="rounded-xl bg-[#f7fafc] border border-gray-200 p-5">
                                ...
                                <p class="text-lg font-bold text-gray-900">NPR {{ number_format($course->fee) }}</p>
                            </div>
                        @endif
                        ...
                        @if ($course->duration_hours)
                            <div class="rounded-xl bg-[#f7fafc] border border-gray-200 p-5">
                                ...
                                <p class="text-lg font-bold text-gray-900">{{ $course->duration_hours }} hours</p>
                            </div>
                        @endif
                ...
                @if ($course->description)
                    <div class="bg-white rounded-xl shadow-[0_5px_15px_rgba(0,0,0,0.08)] border border-gray-200 p-6 md:p-8">
                        <h2 class="text-[1.6rem] font-bold text-[#2c5aa0] mb-4">About This Course</h2>
                ...
                {{-- Other courses from same institution --}}
                @if ($relatedCourses->isNotEmpty())
                    <div>
                        <h2 class="text-[1.6rem] font-bold text-[#2c5aa0] mb-5">More Courses from {{ $inst?->name }}</h2>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            @foreach ($relatedCourses as $related)
                                @include('website.partials.course-card', ['course' => $related])
                            @endforeach
                        </div>
                    </div>
                @endif
            ...
            <aside class="lg:sticky lg:top-28 space-y-6">
                <div class="bg-white rounded-xl shadow-[0_5px_15px_rgba(0,0,0,0.08)] border border-gray-200 p-6">
                    <h3 class="text-xl font-bold text-[#2c5aa0] mb-3">Enroll in This Course</h3>
                    <p class="text-sm text-gray-600 leading-relaxed mb-5">Submit an application and the institution will get in touch with you about enrollment.</p>
                    @auth('student')
                        <a href="{{ $applyUrl }}"
                    ...
                    <div class="space-y-3 text-sm">
                        @if ($course->fee)
                            <div class="flex justify-between gap-4">
                                ...
                                <span class="font-semibold">NPR {{ number_format($course->fee) }}</span>
                            </div>
                        @endif
                        @if ($course->duration_hours)
                            <div class="flex justify-between gap-4">
                                <span class="text-white/70">Duration</span>
                                <span class="font-semibold">{{ $course->duration_hours }} hours</span>
                            </div>
                        @endif
                        @if ($inst)
                            <div class="flex justify-between gap-4">
