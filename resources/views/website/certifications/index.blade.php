...
@section('meta-title', 'Certifications - ' . config('app.name'))
@section('meta-description', 'Browse industry-recognized certifications offered by top institutions. Filter by institution, name, and fee.')
...
<section class="relative overflow-hidden text-white pt-[160px] pb-[60px] bg-gradient-to-br from-[#2c5aa0] to-[#1a365d]">
    ...
    <div class="max-w-[1200px] mx-auto px-5 relative z-10">
        <div class="max-w-[800px] mx-auto text-center">
            <h1 class="text-[3.2rem] leading-[1.2] font-bold mb-5 max-md:text-[2.8rem] max-sm:text-[2.3rem]">Explore Certifications</h1>
            <p class="text-[1.2rem] text-white/90 mb-8">Earn industry-recognized certifications from trusted institutions to advance your career.</p>
            <a href="#certification-list" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold bg-white text-[#2c5aa0] hover:-translate-y-0.5 transition shadow-sm no-underline">
                <i class="fas fa-certificate"></i> Browse Certifications
            </a>
        </div>
    </div>
</section>
...
{{-- Top search bar --}}
<section id="certification-list" class="relative z-10 -mt-10 bg-white rounded-xl shadow-[0_5px_15px_rgba(0,0,0,0.05)] max-w-[1160px] mx-auto px-5 py-10">
    <form method="get" action="{{ route('website.certifications.index') }}">
        <div class="mb-8">
            <h2 class="relative inline-block text-[2.2rem] font-bold text-[#2c5aa0] mb-2">Find a Certification<span class="absolute left-0 -bottom-2 w-20 h-1 bg-[#4299e1] rounded"></span></h2>
        ...
        <div class="grid grid-cols-4 gap-5 mb-6 max-lg:grid-cols-2 max-sm:grid-cols-1">
            <div class="flex flex-col">
                <label class="mb-2 font-semibold text-[#2d3748] text-[0.95rem]">Certification Name</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by certification name"
                ...
                <div class="p-6">
                    ...
                    <div class="pb-5 mb-6 border-b border-gray-200">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <a href="{{ route('website.certifications.index', request()->except(['institution', 'page'])) }}" class="text-[0.8rem] text-[#4299e1] no-underline">Clear</a>
                        </div>
                    ...
                    <div class="pb-5 mb-6 border-b border-gray-200">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <a href="{{ route('website.certifications.index', request()->except(['fee_max', 'page'])) }}" class="text-[0.8rem] text-[#4299e1] no-underline">Clear</a>
                        </div>
                        <input type="number" name="fee_max" value="{{ request('fee_max') }}" min="0" step="1000" placeholder="e.g. 50000"
                               class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:border-[#4299e1] focus:ring-4 focus:ring-[#4299e1]/10 transition">
                    </div>
                    ...
                    {{-- Search --}}
                    <div>
                        <h3 class="text-[1.1rem] font-bold text-[#2c5aa0] mb-3">Certification Name</h3>
            ...
            <div>
                <div class="flex items-center justify-between mb-8 max-md:flex-col max-md:items-start max-md:gap-4">
                    <div class="text-[1.1rem] text-[#2d3748]">
                        Showing <strong class="text-[#2c5aa0]">{{ $certifications->count() }}</strong> of <strong class="text-[#2c5aa0]">{{ $certifications->total() }}</strong> certifications
                    </div>
                    @if(request()->hasAny(['search', 'institution', 'fee_max']))
                        <a href="{{ route('website.certifications.index') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-red-500 transition no-underline">
                ...
                @if ($certifications->isEmpty())
                    <div class="text-center py-20">
                        <div class="w-20 h-20 rounded-full bg-[#4299e1]/10 flex items-center justify-center mx-auto mb-5">
                            <i class="fas fa-certificate text-3xl text-[#4299e1]"></i>
                        </div>
                        <h3 class="text-[1.5rem] font-bold text-gray-600 mb-3">No certifications found</h3>
                        <p class="text-gray-500 mb-6">Try adjusting your filters or search term.</p>
                        <a href="{{ route('website.certifications.index') }}" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl font-semibold bg-[#4299e1] text-white hover:bg-[#2c5aa0] transition no-underline">
                ...
                @else
                    ...
                    @include('website.partials.pagination', ['paginator' => $certifications])
                @endif
            </div>
