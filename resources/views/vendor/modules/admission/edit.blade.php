                ...
                <div x-show="admissionType === 'course'" x-cloak class="mt-6">
                    ...
                    <select id="courses" name="courses[]" multiple class="w-full">
                        ...
                        <option value="{{ $course->id }}"
                            {{ in_array($course->id, old('courses', $selectedCourseIds)) ? 'selected' : '' }}>
...
<script>
    function admissionForm() {
        return {
            admissionType: "{{ old('admission_type', $admission->admission_type) }}",
            grades: [],

            init() {
                const oldGrades = "{!! json_encode(old('grades', $admission->grades ?? [])) !!}";
